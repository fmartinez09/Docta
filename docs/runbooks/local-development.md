...
**Revisión documental:** 2026-09-22. Ejecutar comandos desde la raíz del repositorio.
Esta guía reúne la instalación, el arranque, los endpoints y las verificaciones antes detallados
en el README. Los comandos no fueron ejecutados como parte de la reorganización documental.
Conservar `.env` existente; los valores predeterminados no describen necesariamente el entorno activo.
Consultar [estado actual](../CURRENT_STATE.md) para capacidades y límites.
...
If your configured web origin uses `127.0.0.1:3100`, use `npm run dev:web:local` instead.
...
See the [ingestion operations runbook](ingestion.md) for recovery and diagnostics.
...
Next.js loads the root `.env` server-side. See the [browser runbook](browser-workspace.md)
...
when evidence requires generation. See the [conversation runbook](conversations.md)
...
Synchronize `REDIS_PORT` with `DOCTA_REDIS_URL` as well. `DOCTA_WEB_ORIGIN`, the OIDC callback,
MinIO CORS and the actual web port must describe the same origin.
